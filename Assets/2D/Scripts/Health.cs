...
{
    [SerializeField] public float health = 100;              // Current health amount
    ...
    [SerializeField] private UnityEvent onDamage;            // Event triggered when damage is applied
    [SerializeField] private UnityEvent onDeath;             // Event triggered when the entity dies
    ...
    public void ApplyDamage(float damage)
    {
        ...
        if (health <= 0)
        {
            onDeath?.Invoke(); // Trigger damage event again for death
            Die();
        }
        else
        {
            onDamage?.Invoke(); // Trigger damage event
